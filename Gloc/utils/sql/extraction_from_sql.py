...
def extract_partial_template_from_sql(sql, schema={}, use_corenlp=False, use_duckdb=False):
	"""
		This function extracts a partial template from a given SQL query. For example, given the SQL query:
			SELECT name FROM students WHERE age > 20
		the function might return a partial template like 
			SELECT [MASK] FROM [MASK] WHERE [MASK] > [VALUE]
		
		Parameters:
		sql (str): The SQL query from which to extract the template.
		schema (dict, optional): A dictionary representing the schema of the database. Defaults to an empty dictionary.
		
		Returns:
		list: A list of tokens representing the partial template extracted from the SQL query.
	"""
	toks = tokenize(
				string=sql, 
